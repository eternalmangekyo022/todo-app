...
{/* 20% */}     <Button disabled={complete} style={{ width: '20%', left: '1rem', position: 'absolute' }} onClick={deleteTodo}><IconTrash /></Button>
...
{/* 20% */}     <Button style={{ width: '20%', right: '1rem', position: 'absolute' }} onClick={() => dispatch({ type: "update", id, complete: !complete })}>{complete ? <IconX/> : <IconCheck/>}</Button>
            </Stack>
            <Modal style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} centered withCloseButton={true} opened={active} onClose={activeWrapper(false)} >
                <Center>
                    <Text>{text}</Text>
                </Center>
            </Modal>
